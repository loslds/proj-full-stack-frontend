...
import type { DropdownOption } from "../../components/sidebar/Dropdown";
...
export function useSystemTables(enabled: boolean = true): UseSystemTablesReturn {
  ...
  React.useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!existing.has("systables") && !missing.has("systables")) missing.add("systables");
        for (const t of existing) missing.delete(t);

        const optsExisting: DropdownOption[] = Array.from(existing)
          .sort((a, b) => a.localeCompare(b))
          .map((t) => ({ value: t, label: `✅ ${t}` }));

        const optsMissing: DropdownOption[] = Array.from(missing)
          .sort((a, b) => a.localeCompare(b))
          .map((t) => ({ value: t, label: `⚠️ ${t}` }));
