...
const Home: React.FC = () => {
  ...
  // 🔧 IMPORTANTE: este state agora controla EXCLUSIVAMENTE o modal de verificação (fechável)
  ...
  ////////////////////////////////////////////////////////////////////////////////
  // trava global: se qualquer modal estiver aberto, a chave-master não pode abrir
  // const anyModalOpen =
  ...
  // // Hook da Chave-Master (toda a lógica saiu da Home)
  // const cm = useChaveMaster({
  //   enabled: true,

  //   // bloqueia se modal aberto ou sistema não pronto
  //   blocked: anyModalOpen || !state.initsys || !state.chkdb,

  //   // validação da chave (sua regra atual)
  //   validateKey: (k: string) => CheckDateToCecular(k),

  //   // tentativas máximas
  //   maxKeyFails: 5,

  //   // retorno final: true = sucesso | false = falha
  //   onResult: (ok: boolean) => {
  //     if (ok) {
  ...
  //     } else {
  //       setMsgPanelBottom("Chave inválida (limite atingido).");
  //     }

  //     setTimeout(() => setMsgPanelBottom(""), 3000);
  //   },
  // });
/////////////////////////////////////////////////////
  ...
  React.useEffect(() => {
    ...
    async function runSystemCheck() {
      try {
        // Abre modal para informar verificação, mas agora ele É FECHÁVEL.
        ...
        const messages: string[] = [`🧭 Modo do sistema: ${data.mode}`, `📦 Tabelas encontradas: ${data.existingTables.length}`];
  ...
  React.useEffect(() => {
    ...
    if (state.initsys) {
      if (state.chkdb) {
        if (state.logado || state.chvkey) {
          if (state.logado) {
            ...
            // acesso com a Chave Master para entrar no Sistema.
        ...
        setShowSystemCheckModal(true); // mantém modal informativo (fechável)
      ...
      // 🔧 NÃO amarrar UI (modal) a initsys. initsys é verdade do sistema; modal é UI.
      dispatch({ type: UseAcessoActions.set_INITSYS, payload: false });
      dispatch({ type: UseAcessoActions.set_CHVKEY, payload: false });
      dispatch({ type: UseAcessoActions.set_ID_NIVEL, payload: 0 });
      dispatch({ type: UseAcessoActions.set_ACAO, payload: '' });
      ...
      // Mantemos o state antigo, mas o modal real agora é showsystemcheckmodal
    ...
    <ThemeProvider theme={theme}>
      ...
      >
        ...
        <ContentSidePagePanelBotton bordas="3px" open={true} pwidth="100%">
          ...
          {/** INICIO DO CONTAINER DOS BOTÕES DA PAGINA */}    
        ...
        {/** FIM DO CONTAINER DOS BOTÕES DA PAGINA */}    
        
        {/** INICIO DOS MODAIS CONTIDOS NA PAGINA */}    
        {cardnegadopage ? (
          <PageModal
            ptop={'1%'}
            pwidth={'30%'}
          ...
          >
            <CardImgNeg imgcard={pnl_negado} pminheight={'100px'} pwidth={'100px'} onclickimg={() => setCardNegadoPage(false)} />
          </PageModal>
        ) : null}
        ...
        {notOperation ? (
          ...
          >
            <CardImgNeg imgcard={pnl_negado} pminheight={'120px'} pwidth={'120px'} onclickimg={() => setNotOperation(false)} />
            <form>
        ...
        {/** FIM DOS MODAIS CONTIDOS NA PAGINA */}    
